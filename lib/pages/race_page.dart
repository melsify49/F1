...
import '../services/simulation_service.dart'; // ✅ تحديث الخدمة
import '../models/race_strategy.dart';
import '../models/race_result.dart';
import '../utils/constants.dart';
import '../widgets/strategy_panel.dart';
import '../widgets/live_race_view.dart'; // ✅ تحديث الواجهة
...
class RacePage extends StatefulWidget {
  ...
  const RacePage({super.key, required this.raceEvent, required this.round});
...
class _RacePageState extends State<RacePage> {
  ...
  late EnhancedSimulationService _simulationService;
  ...
  void initState() {
    ...
    _simulationService = EnhancedSimulationService();
    _currentWeather = widget.raceEvent.baseWeather;
    _currentRace = widget.round;
    _currentFuel = _currentStrategy.fuelLoad.toDouble();
  ...
  void _setupSimulationListener() {
    _simulationService.raceStream.listen((update) {
      ...
      setState(() {
        ...
        if (update['type'] == 'race_finished') {
        ...
        } else if (update['type'] == 'lap_update') {
          ...
          _currentFuel = update['fuelLevel'] ?? _currentFuel;
          _currentTireWear = update['tireWear'] ?? _currentTireWear;
  ...
  void _onRaceFinished(Map<String, dynamic> result) {
    ...
    final saveManager = Provider.of<SaveManager>(context, listen: false);
    final raceEvent = widget.raceEvent;

    // إنشاء نتيجة السباق مع جميع البيانات المطلوبة
    final raceResult = RaceResult(
      raceId: 'race_${DateTime.now().millisecondsSinceEpoch}',
      raceName: raceEvent.name,
      round: widget.round,
      teamId: saveManager.playerTeam!.id,
      finalPosition: result['finalPosition'] ?? 0,
      driver1Position:
          result['driver1Position'] ?? result['finalPosition'] ?? 0,
      driver2Position:
          result['driver2Position'] ?? result['finalPosition'] ?? 0,
      ...
      completedLaps: result['completedLaps'] ?? raceEvent.totalLaps,
      pitStopLap: result['pitStopLap'] ?? 20,
      weather: _parseWeather(result['finalWeather']),
      difficulty:
          double.tryParse(result['difficulty']?.toString() ?? '0.5') ?? 0.5,
      raceStandings: List<Map<String, dynamic>>.from(
        result['raceStandings'] ?? [],
      ),
      driver1Name: saveManager.playerTeam!.driver1.name,
      driver2Name: saveManager.playerTeam!.driver2.name,
    ...
    // الحصول على جميع الفرق الحقيقية من TeamManager
    final allTeams = _getAllTeams();

    // استخدام الدالة المحدثة في SaveManager
    saveManager.completeRace(raceResult, allTeams);
  ...
  List<Team> _getAllTeams() {
    final saveManager = Provider.of<SaveManager>(context, listen: false);
    final playerTeam = saveManager.playerTeam!;
    
    // الحصول على جميع الفرق المسبقة
    final presetTeams = TeamManager.getPresetTeams();
    
    // إذا كان فريق اللاعب موجوداً بالفعل في الفرق المسبقة، استخدم الفرق المسبقة كما هي
    if (presetTeams.any((team) => team.id == playerTeam.id)) {
      return presetTeams;
    }
    
    // إذا كان فريق اللاعب جديداً (مخصص)، أضفه للقائمة
    return [playerTeam, ...presetTeams];
  }
  ...
  void _startRace() async {
    ...
    await _simulationService.simulateEnhancedRace(
      playerTeam: team,
  ...
  List<Team> _createSmartAITeams(Team playerTeam, int currentRace) {
    final presetTeams = TeamManager.getPresetTeams()
        .where((team) => team.id != playerTeam.id)
        .toList();
    final aiDifficulty = _calculateAIDifficulty(currentRace);

    return presetTeams.map((teamData) {
      final performanceMultiplier = _getAIPerformanceMultiplier(
        presetTeams.indexOf(teamData),
        aiDifficulty,
      );
  ...
  void _handleStrategyChangeDuringRace(RaceStrategy newStrategy) {
    ...
    _raceEvents.add(
      "🔄 تغيير في الإستراتيجية: ${_getStrategyChangeDescription(newStrategy)}",
    );
  }
  ...
  String _getStrategyChangeDescription(RaceStrategy newStrategy) {
    ...
    if (newStrategy.aggression != _currentStrategy.aggression) {
      changes.add(
        "العدوانية إلى ${_getAggressionName(newStrategy.aggression)}",
      );
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _isSimulating ? _buildModernLiveRaceView() : _buildStrategySetup(),
    );
  }
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      _buildRaceInfoItem(
                        'الصعوبة',
                        widget.raceEvent.difficultyLevel,
                      ),
                    ],
                  ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            _buildStrategyItem('نوع الإطارات', _getTireName(_currentStrategy.tireChoice), _getTireEmoji(_currentStrategy.tireChoice)),
            _buildStrategyItem('لفة Pit Stop', '${_currentStrategy.pitStopLap}', '⏱️'),
            _buildStrategyItem('مستوى العدوانية', _getAggressionName(_currentStrategy.aggression), _getAggressionEmoji(_currentStrategy.aggression)),
  ...
  // ✅ استخدام ModernLiveRaceView مع البيانات المطلوبة
  Widget _buildModernLiveRaceView() {
    return ModernLiveRaceView(
      strategy: _currentStrategy,
      onStrategyChange: _handleStrategyChangeDuringRace,
      currentWeather: _currentWeather,
      totalLaps: widget.raceEvent.totalLaps,
